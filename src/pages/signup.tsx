  ...
  const sendOtp = async () => {
    try {
      ...
      if (!res.ok) {
        setStatus({
          type: "error",
          message: data.error || "Otp not sent. Please try again."
        });
        return false;
      }
      return true;
        ...
        <div className="p-4 m-4 w-[80%] sm:w-[50%] lg:w-[30%] shadow-lg rounded-2xl bg-teal-50">
          ...
          <form onSubmit={(e) => handleSignUp(e)} className="p-2 w-full space-y-2">
            ...
            <div className="text-center">
              {!otpSent ? <button
                type="submit"
                className={`bg-green-400 ${loading ? "cursor-not-allowed" : ""} hover:bg-green-500 rounded-sm p-1 py-2 w-[40%] mt-8 my-5 focus:scale-95 transition-all sm:text-lg font-semibold text-white`}>{loading ? "Please wait.." : "SignUp"}</button> :
                <button
                type="button"
                  onClick={() => signUp()}
